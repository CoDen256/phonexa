...
def process_entry(url, fro, to, sound, output_dir: str) -> None:
    ...
    segment = audio[fro:to]
    out_path = os.path.join(output_dir, f"{sound}_vowel.mp3")
    save_segment(segment, out_path)

    # 5. Optionally remove the raw download to save space
    # os.remove(raw_path)
process_entry("https://audio.dict.cc/speak.audio.v2.php?error_as_text=1&type=mp3&id=1550429&lang=no_rec_ip&lp=DENO", 
              0,150,
              "yndi", 
              "./no")
